...
class VectorViewer:
    ...
    def __init__(self, width, height):
        ...
        self.lightPosition = np.array([-500.0, 600.0, -1300.0])
    ...
    def calculate(self):
        ...
        for VectorObj in (vobj for vobj in self.VectorObjs if vobj.visible == 1 and vobj != self.groundObject):
            ...
            VectorObj.updateSurfaceAngleToLightSource(self.lightPosition, self.VectorPos.position)
            VectorObj.updateSurfaceColorShade()
            if VectorObj.shadow == 1:
                # calculate shadow data
                VectorObj.updateShadow(self.viewerAngles, self.lightPosition, self.VectorPos.position, self.objMinZ, self.zScale, self.midScreen)
...
class VectorObject:
    ...
    def __init__(self):
        ...
        self.shadowNodes = np.zeros((0, 4))                 # nodes will have unrotated X,Y,Z coordinates plus a column of ones for position handling
    ...
    def updateSurfaceAngleToLightSource(self, lightPosition, vectorPosition):
        ...
        for surface in (surf for surf in self.surfaces if (surf.visible == 1 and self.minShade < 1.0) or self.shadow == 1):
            surface.setLightSourceVector(self.rotatedNodes[surface.nodes[1], 0:3] - vectorPosition[0:3] - lightPosition)
...
if __name__ == '__main__':
    ...
    for angles in root.iter('vectorangles'):
        ...
        if ang.angName == "viewer":
            ...
            vv.viewerAngles.rotate = ([0.0, 0.2, 0.0])
