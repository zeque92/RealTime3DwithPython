...
class TheWorld:
    """
    Shows a real-time calculated, light source shaded city with building shadows.
    ...
    def __init__(self, screen, target_fps):
        ...
        self.zScale = self.width * 0.7                  # Scaling for z coordinates
        ...
        # initialize font and set font size
        if self.height >= 800:
        ...
        # set up timers
        self.timer_names.append("movements")
        self.timer_names.append("positions")
        self.timer_names.append("rotation matrix")
        self.timer_names.append("rotate")
        self.timer_names.append("transform 2D")
        self.timer_names.append("crossprodvec")
        self.timer_names.append("object angles")
        self.timer_names.append("shadow calc")
        self.timer_names.append("screen clear")
        self.timer_names.append("ground + clear")
        self.timer_names.append("draw blends")
        self.timer_names.append("draw shadows")
        self.timer_names.append("draw surfaces")
        ...
        self.prepareWorld("vectordata.xml")
            ...
            if VectorMovement == self.viewerMovement:
                ...
                for (node_num, VectorObj) in self.VectorPos.objects:
                    if VectorObj.movement == VectorMovement:
                        self.VectorPos.nodes[node_num, 0:3] = (1.0 - mult) * VectorMovement.moveSeries[0:3, loop_pos] \
                            + mult * VectorMovement.moveSeries[0:3, loop_pos + 1]
    ...
    def calculate(self):
        ...
        for VectorObj in (vobj for vobj in self.VectorObjs if vobj.visible == 1 and vobj != self.groundObject):
            ...
            if VectorObj.shadow == 1:
                ...
                VectorObj.updateShadow(self.viewerMovement.angles, self.lightNode, light_position, VectorObj.nodePosition[0:3],
                                       self.objMinZ, self.zScale, self.midScreen)
        ...
        for prio_nr in self.VectorObjPrios:
            ...
            for VectorObj in (vobj for vobj in self.VectorObjs if vobj.visible == 1 and vobj.prio == prio_nr):
                ...
                if VectorObj == self.groundObject:
                    ...
                    node_list = self.cropEdges([transNodes[0, 0:2], transNodes[0, 2:4], transNodes[1, 2:4], transNodes[1, 0:2]], False, True)  # X cropped
                    ...
                    if self.groundBlendPrio is not None:
                        ...
                        node_list = self.cropEdges([transNodes[1, 0:2], transNodes[1, 2:4], transNodes[self.groundShadeNr + 1, 2:4],
                                                    transNodes[self.groundShadeNr + 1, 0:2]], False, True)  # X already cropped
                        ...
                        for i in range(self.groundShadeNr - 1):
                            node_list = self.cropEdges([transNodes[i+1, 0:2], transNodes[i+1, 2:4], transNodes[i+2, 2:4],
                                                        transNodes[i+2, 0:2]], False, True)  # X already cropped
    ...
    def prepareWorld(self, datafile):

        # read data file defining angles, movements and objects
        vecdata = et.parse(datafile)
        ...
        for movements in root.iter('movement'):
            ...
            for angles in self.VectorAnglesList:
            ...
            for timedata in movements.iter('timeseries'):
                ...
                num_times = int((mov_times[0, -1] - mov_times[0, 0]) * self.target_fps / movSpeed)
            ...
            for loopdata in movements.iter('loop'):
                mov.loopStart = int(float(loopdata.findtext("loopstart", default="0")) * self.target_fps / movSpeed)
                mov.loopEnd = int(float(loopdata.findtext("loopend", default="0")) * self.target_fps / movSpeed)
                mov.loopPos = int(float(loopdata.findtext("looppos", default="0")) * self.target_fps / movSpeed)
                break  # only one loop accepted

            self.addVectorMovementList(mov)
            # define this (last added) movement as the viewer movement if so specified.
            if viewer == "1":
                self.viewerMovement = self.VectorMovementList[-1]
        ...
        for vecobjs in root.iter('vectorobject'):
            ...
            if ground == "1":
                self.groundBlendPrio = int(vecobjs.findtext("blendprio", default=None))
            ...
            if not vobj.prio in self.VectorObjPrios:
                self.VectorObjPrios.append(vobj.prio)
            # add the object
            self.addVectorObj(vobj)

            # add it to a movement's list of objects, if so specified
            if self.VectorObjs[-1].movement is not None:
                self.VectorObjs[-1].movement.addObject(self.VectorObjs[-1])
            # define this (last added) object as light source or ground if so specified
            if lightsource is not None:
                self.lightObject = self.VectorObjs[-1]
            # define this (last added) object as ground if so specified
            if ground is not None:
                self.groundObject = self.VectorObjs[-1]

        # sort prio list
        self.VectorObjPrios.sort(reverse=True)
...
class VectorObject:
    ...
    def updateSurfaceColorShade(self):
        ...
        for surface in (surf for surf in self.surfaces if surf.visible == 1):
            ...
            if surface.showBack == 1:
                surface.setBackColorShade(self.minShade)
    ...
    def updateShadow(self, viewerAngles, lightNode, light_position, obj_pos, objMinZ, zScale, midScreen):
        ...
        for node_num in range(len(node_list)):
            ...
            if obj_pos[1] + self.objRotatedNodes[node, 1] > 3 and obj_pos[1] + self.objRotatedNodes[node, 1] < (lightNode[1] - 3):
                ...
                # the projection multiplier if based on above diff_node where Y=0 is ground, but it can be applied directly to rotatedNodes as well,
                # skipping one rotation step.
                diff_rotated_node = self.rotatedNodes[node, :] - light_position
                self.shadowRotatedNodes = np.vstack((self.shadowRotatedNodes,
                                                     np.hstack((light_position + diff_rotated_node * (lightNode[1] / -diff_node[1]), 1))))
...
class VectorObjectSurface:
    ...
    def setAngleToViewer(self, vec_Viewer):
        if self.crossProductLen > 0 and vec_Viewer.any() != 0:
            # instead of true angle calculation using asin and vector lengths, a simple np.vdot is sufficient to find the sign
            #     (which defines if surface is visible)
    ...
    def setAngleToLightSource(self):
        if self.crossProductLen > 0 and self.lightSourceVector.any() != 0:
            # instead of true angle calculation using asin and vector lengths, a simple np.vdot is sufficient to find the sign
            #     (which defines if surface is shadowed)
            # self.angleToLightSource = math.asin(np.dot(self.crossProductVector, self.lightSourceVector)
            #     / (self.crossProductLen * np.linalg.norm(self.lightSourceVector))) / (np.pi / 2)
    ...
    def setColorShade(self, minShade):
        if self.angleToLightSource <= 0:
            ...
            self.colorShade = minShade + (1.0 - minShade) * math.asin(self.angleToLightSource
                                                                      / (self.crossProductLen * self.vectorLen(self.lightSourceVector))) / (np.pi / 2)

    def setBackColorShade(self, minShade):
        if self.angleToLightSource >= 0:
            self.backColorShade = minShade
        else:
            self.backColorShade = minShade + (1.0 - minShade) * -math.asin(self.angleToLightSource
                                                                           / (self.crossProductLen * self.vectorLen(self.lightSourceVector))) / (np.pi / 2)
...
class VectorAngles:
    ...
    def setRotateAngles(self):
        ...
        for i in range(3):
            if self.rotateAngles[i] >= 360:
                self.rotateAngles[i] -= 360
            if self.rotateAngles[i] < 0:
                self.rotateAngles[i] += 360
