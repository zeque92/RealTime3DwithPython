    ...
    def raycast(self, screen, block_size):
        ...
        if self.mode == 2:
            for k in range(10):
                ...
                #                     (self.height - block_size * self.map_size[1]) // 2 + pos_y * block_size], dtype=np.int)
                for j in range(np.shape(x_coord)[1]):
                    ...
                    use_pos = np.array([(self.width - block_size * self.map_size[0]) // 2 + x_coord[i, j] * block_size,
                                        (self.height - block_size * self.map_size[1]) // 2 + x_coord_y_adj[i, j] * block_size], dtype=np.int)
            ...
            for k in range(10):
                ...
                for j in range(np.shape(y_coord)[1]):
                    ...
                    use_pos = np.array([(self.width - block_size * self.map_size[0]) // 2 + y_coord_x_adj[i, j] * block_size,
                                        (self.height - block_size * self.map_size[1]) // 2 + y_coord[i, j] * block_size], dtype=np.int)
                    if horiz_grid[i, j] == 1:
